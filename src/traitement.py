# -*-coding:UTF-8 -*
"""Permet de faire une vérification du délais de notification"""
...
def comparaison_heure(capteur_valide):
    ...
    for amac in capteur_valide:
        ecart_notif = datetime.now().replace(microsecond=0) - \
            LISTE_NOTIFICATION_ASSOCIE[amac]
        if ecart_notif.seconds > 60:
            capteur_notifie.append(amac)
            LISTE_NOTIFICATION_ASSOCIE[amac] = datetime.now(
